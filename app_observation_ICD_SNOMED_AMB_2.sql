...
/*******************************************************************************
Name: app_observation_ICD_SNOMED_AMB_2.sql
...
Description: This script is the 2nd it a two-part process.  It is used in conjunction with 
	(and following) pull_observation_ICD_SNOMED_AMB_2.sql. 

	Its purpose is to join the data in [OMOP_Clarity].[OBSERVATION_ClarityAMB_SNOMED_ICD] to the OMOP concept table
...
SELECT DISTINCT OBSERVATION_ClarityAMB_SNOMED_ICD.person_id
	,t_sno_concept.CONCEPT_ID AS OBSERVATION_concept_id
	,CONVERT(DATE, OBSERVATION_ClarityAMB_SNOMED_ICD.CONTACT_DATE) AS OBSERVATION_date
	,OBSERVATION_ClarityAMB_SNOMED_ICD.CONTACT_DATE AS OBSERVATION_datetime
	,38000282 AS OBSERVATION_type_concept_id --Chief complaint
	,NULL AS value_as_number
	-- returns ICD10 codes after switch date-----
	,CASE 
		WHEN OBSERVATION_ClarityAMB_SNOMED_ICD.CONTACT_DATE >= '2015-10-01'
...
FROM OMOP_Clarity.OBSERVATION_ClarityAMB_SNOMED_ICD

INNER JOIN omop.visit_occurrence
	ON OBSERVATION_ClarityAMB_SNOMED_ICD.PAT_ENC_CSN_ID = visit_occurrence.[visit_source_value]

LEFT JOIN OMOP.provider
	ON OBSERVATION_ClarityAMB_SNOMED_ICD.visit_PROV_ID = provider.[provider_source_value]

--------Concept Mapping--------------------
INNER JOIN T_SOURCE AS t_sno_source
	ON OBSERVATION_ClarityAMB_SNOMED_ICD.SNOMED_code = t_sno_source.concept_code

INNER JOIN T_ICD_SOURCE AS t_icd9_source
	ON OBSERVATION_ClarityAMB_SNOMED_ICD.icd9_code = t_icd9_source.concept_code

INNER JOIN T_ICD_SOURCE AS t_icd10_source
	ON OBSERVATION_ClarityAMB_SNOMED_ICD.icd10_code = t_icd10_source.concept_code
