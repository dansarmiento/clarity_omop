...
SELECT DISTINCT 
	...
	,CASE 
		WHEN CONDITION_OCCURRENCE_ClarityHosp_ICD.CONTACT_DATE >= '2015-10-01'
			THEN t_icd10_concept.CONCEPT_ID
		ELSE t_icd9_concept.CONCEPT_ID
		END AS OBSERVATION_concept_id

	,CONVERT(DATE, CONDITION_OCCURRENCE_ClarityHosp_ICD.CONTACT_DATE) AS condition_start_date
	,CONDITION_OCCURRENCE_ClarityHosp_ICD.CONTACT_DATE AS condition_start_datetime
	,CONVERT(DATE, CONDITION_OCCURRENCE_ClarityHosp_ICD.[HOSP_DISCH_TIME]) AS condition_end_date
...
FROM OMOP_Clarity.CONDITION_OCCURRENCE_ClarityHosp_ICD
	...
	LEFT OUTER JOIN omop.provider
		ON CONDITION_OCCURRENCE_ClarityHosp_ICD.PROV_ID = provider.[provider_source_value]
...
-------------------------------------------
WHERE 	
	(CONDITION_OCCURRENCE_ClarityHosp_ICD.CONTACT_DATE >= '2015-10-01' and t_icd10_concept.CONCEPT_ID IS NOT NULL)   
	or 
	(CONDITION_OCCURRENCE_ClarityHosp_ICD.CONTACT_DATE < '2015-10-01' and t_icd9_concept.CONCEPT_ID IS NOT NULL   )
