...
/*******************************************************************************
Name: app_condition_occurrence_SNOMED_ICD_AMB_2.sql
...
Description: This script is the 2nd it a two-part process.  It is used in conjunction with 
	(and following) pull_condition_occurrence_SNOMED_ICD_AMB_2.sql. 

	Its purpose is to join the data in [OMOP_Clarity].[CONDITION_OCCURRENCE_ClarityAMB_SNOMED_ICD] to the OMOP concept table
...
SELECT DISTINCT CONDITION_OCCURRENCE_ClarityAMB_SNOMED_ICD.person_id
	,t_sno_concept.CONCEPT_ID AS condition_concept_id
	,CONVERT(DATE, CONDITION_OCCURRENCE_ClarityAMB_SNOMED_ICD.CONTACT_DATE) AS condition_start_date
	,CONDITION_OCCURRENCE_ClarityAMB_SNOMED_ICD.CONTACT_DATE AS condition_start_datetime
	,CONVERT(DATE, CONDITION_OCCURRENCE_ClarityAMB_SNOMED_ICD.CONTACT_DATE) AS condition_end_date
	,CONDITION_OCCURRENCE_ClarityAMB_SNOMED_ICD.CONTACT_DATE AS condition_end_datetime
	,CASE 
	...
	,CASE 
		WHEN CONDITION_OCCURRENCE_ClarityAMB_SNOMED_ICD.CONTACT_DATE >= '2015-10-01'
...
FROM OMOP_Clarity.CONDITION_OCCURRENCE_ClarityAMB_SNOMED_ICD

	INNER JOIN OMOP.visit_occurrence
		ON CONDITION_OCCURRENCE_ClarityAMB_SNOMED_ICD.PAT_ENC_CSN_ID = visit_occurrence.visit_source_value

	LEFT JOIN OMOP.provider
		ON CONDITION_OCCURRENCE_ClarityAMB_SNOMED_ICD.visit_PROV_ID = provider.[provider_source_value]

	--------Concept Mapping--------------------
	INNER JOIN T_SOURCE AS t_sno_source
		ON CONDITION_OCCURRENCE_ClarityAMB_SNOMED_ICD.SNOMED_code = t_sno_source.concept_code

	INNER JOIN T_ICD_SOURCE AS t_icd9_source
		ON CONDITION_OCCURRENCE_ClarityAMB_SNOMED_ICD.icd9_code = t_icd9_source.concept_code

	INNER JOIN T_ICD_SOURCE AS t_icd10_source
		ON CONDITION_OCCURRENCE_ClarityAMB_SNOMED_ICD.icd10_code = t_icd10_source.concept_code
